...
\only<6-9>{
  \path[draw,->,very thick,darkyellow] (7,0) -- (7,2);}
...
%  \node[current process,fill=colora0] (hz_0) at (z_0.center) {};}
  \path[draw,->,very thick,darkgreen] (7.3,2) -- (7.3,0);}
...
%  \node[current process,fill=colora0,inner sep=0,minimum size=7pt,draw=none] (hz_1bis) at (z_1.center) {};}
  \path[draw,->,very thick,darkyellow] (7,0) -- (7,0.9);
  \path[draw,->,very thick,darkyellow] (7,2) -- (7,1.1);
  \path[draw,->,very thick,darkgreen] (7.3,0) -- (7.3,0.9);
  \path[draw,->,very thick,darkgreen] (7.3,2) -- (7.3,1.1);}
...
% Arc noir
\only<2-12>{\path (a) edge[inf] node[elabel,above right=-2.4em,font=\small] {\begin{tabular*}{3cm}{c}%
...
% Arc coloré
\only<13->{\path (a) edge[act,very thick,notsodarkgreen] node[elabel,above right=-2.4em,font=\small] {\begin{tabular*}{3cm}{c}%
$\color{white}\left\{\color{black}\text{\begin{tabular*}{2cm}{l}%
...
\pause
  \item Change the active process of the regulator [\ex{$a_0$}, \ex{$a_1$}] and watch the \tval{evolution}.
\pause[9]
